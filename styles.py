...
input,textarea,input *,textarea *,div[data-baseweb="select"],div[data-baseweb="select"] *,div[data-baseweb="input"],div[data-baseweb="input"] *{color:#020617!important;-webkit-text-fill-color:#020617!important}div[data-baseweb="select"]>div,div[data-baseweb="input"]>div{background:#fff!important;color:#020617!important;border:1px solid #cbd5e1!important}div[data-baseweb="popover"],div[data-baseweb="popover"] *,ul[role="listbox"],ul[role="listbox"] *,div[role="option"],div[role="option"] *,li[role="option"],li[role="option"] *{background:#fff!important;color:#020617!important;-webkit-text-fill-color:#020617!important}
...
.stats-strip{display:grid;grid-template-columns:repeat(6,1fr);background:rgba(15,23,42,.62);border:1px solid #334155;border-radius:10px;margin-bottom:10px;overflow:hidden}.stat-cell{padding:8px 10px;border-right:1px solid #334155}.stat-cell:last-child{border-right:none}.stat-label{font-size:.7rem;color:#94a3b8;text-transform:uppercase;margin-bottom:3px}.stat-value{font-size:.92rem;color:#f8fafc;font-weight:700}.timeframe-shell{margin:6px 0 10px;padding:6px;background:rgba(15,23,42,.52);border:1px solid #334155;border-radius:10px}.timeframe-chip{font-size:.78rem;font-weight:800;border-radius:7px;padding:5px 9px;margin-right:6px;border:1px solid #475569;color:#cbd5e1;background:#1e293b}.timeframe-chip-active{background:#60a5fa;color:#020617;border-color:#93c5fd}.timeframe-caption{font-size:.75rem;color:#94a3b8;margin-right:8px}.bubble-footer{font-size:.75rem;color:#bfdbfe!important;border-top:1px solid rgba(191,219,254,.34);margin-top:8px;padding-top:8px}
</style>
"""
...
def get_bubble_header_html(ticker: str, company_name: str, price: float, price_change: float, pct_change: float, market_status: str, is_active: bool = False, last_update: datetime | None = None) -> str:
    ...
    return f'<div class="stock-panel-header"><div class="stock-id"><span class="stock-ticker">{ticker}</span><span class="company-name">{company_name}</span>{active}</div><div class="price-row"><span class="stock-price">{price:,.2f}</span><span class="currency">USD</span><span class="{change_class}">{sign}{price_change:,.2f}</span><span class="{change_class}">({sign}{pct_change:.2f}%)</span></div><div class="market-row"><span class="{market_class}">{market_status}</span><span>{last_update.strftime("%b %d, %Y %I:%M %p")}</span></div></div>'
